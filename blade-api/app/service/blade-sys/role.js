...
class RoleService extends Service {
  async add(params) {
    ...
    const [ result, status ] = await this.ctx.model.BladeRole._add(params);
    if (status) {
  ...
  async update(params) {
    const data = await this.ctx.model.BladeRole._update(params);
    if (data) {
  ...
  async delete(params) {
    const status = await this.ctx.model.BladeRole._delete(params);
    if (status) {
