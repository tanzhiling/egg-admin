...
class AuthService extends Service {
  async token(params) {
    const hasUser = await this.ctx.model.BladeUser._findOne({ username: params.username }, true);
    if (hasUser) {
      const password = this.ctx.helper.md5(params.password);
      const data = await this.ctx.model.BladeUser._findOne({ username: params.username, password }, true);
      if (data) {
